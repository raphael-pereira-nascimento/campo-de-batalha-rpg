...
export const api = {
  ...
  getCharacter: (id) => auth(`/api/characters/${id}`),
  equipItem: (id, slot, itemId) => auth(`/api/characters/${id}/equip`, { method: 'POST', body: JSON.stringify({ slot, itemId }) }),
  getGameData: () => request('/api/gamedata'),
  getWallet: () => auth('/api/wallet'),
  getShop: () => auth('/api/shop'),
  buyItem: (itemId, characterId) => auth('/api/shop/buy', { method: 'POST', body: JSON.stringify({ itemId, characterId }) }),
